...
parser = argparse.ArgumentParser(description='Calibrate By OpenCV')
parser.add_argument('-x', '--num_x', type=int, help='X轴角点个数', default=11)
parser.add_argument('-y', '--num_y', type=int, help='Y轴角点个数', default=8)
...
# 获取标定板角点的位置
objp = np.zeros((objp_x * objp_y, 3), np.float32)
...
#   print(corners)
    ...
    if ret:
        ...
        i += 1
        cv2.imwrite('corner_img'+str(i)+'.jpg', img)
        cv2.waitKey(1500)
